...
with a large number of observations or clusters [^1].
...
as the next result, which slightly generalizes the Theorem 1 in @PuTi18, shows.[^3]

[^3]: In particular, @PuTi18 require $A_s$ to be the pseudo-inverse. We show that it's sufficient for $A_s$ to be a generalized inverse.
...
result, a version of which was suggested to us by Ulrich Müller, which allows us
to compute $a_{s}$ by computing a spectral decomposition of a $p\times p$
matrix.
